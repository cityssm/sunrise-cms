    ...
    await describe('buildBurialSiteName', async () => {
        ...
        await it('returns a burial site name with all available segments', () => {
            ...
            if (segmentConfig.includeCemeteryKey ?? false) {
                assert.ok(burialSiteName.startsWith('CEM'), `Burial site name should start with cemetery key: ${burialSiteName}`);
            }
            else {
                assert.ok(!burialSiteName.startsWith('CEM'), `Burial site name should not start with cemetery key: ${burialSiteName}`);
            }
            if (segmentConfig.segments['1']?.isAvailable ?? false) {
                assert.ok(burialSiteName.includes('S1'), `Burial site name should include segment 1: ${burialSiteName}`);
            }
            if (segmentConfig.segments['2']?.isAvailable ?? false) {
                assert.ok(burialSiteName.includes('S2'), `Burial site name should include segment 2: ${burialSiteName}`);
            }
            if (segmentConfig.segments['3']?.isAvailable ?? false) {
                assert.ok(burialSiteName.includes('S3'), `Burial site name should include segment 3: ${burialSiteName}`);
            }
            if (segmentConfig.segments['4']?.isAvailable ?? false) {
                assert.ok(burialSiteName.includes('S4'), `Burial site name should include segment 4: ${burialSiteName}`);
            }
            if (segmentConfig.segments['5']?.isAvailable ?? false) {
                assert.ok(burialSiteName.includes('S5'), `Burial site name should include segment 5: ${burialSiteName}`);
            }
        });
    });
});
