...
await describe('helpers.i18n', async () => {
  await describe('i18next initialization', async () => {
    await it('i18next is initialized', () => {
      ...
      assert.ok(
        ...
        'i18next.t should be a function'
      )
    })
    ...
    await it('has correct fallback language', () => {
      ...
      assert.ok(
        Array.isArray(fallbackLng) || typeof fallbackLng === 'string',
        'Fallback language should be a string or an array'
      )
      ...
      if (Array.isArray(fallbackLng)) {
        assert.strictEqual(
          fallbackLng.length,
          1,
          'Fallback languages array should contain exactly one language'
        )
        assert.ok(
          fallbackLng.includes('en'),
          'Fallback languages should include English'
        )
      } else {
        assert.strictEqual(
          fallbackLng,
  ...
  await describe('i18next translation', async () => {
    ...
    await it('can use fallback language when translation is missing', () => {
      ...
      assert.ok(
        translated,
        'Should return a translation using fallback if needed'
      )
    })
  })
  ...
  await describe('i18next language changing', async () => {
    await it('can change the current language', async () => {
      ...
      assert.strictEqual(
        i18next.language,
        'de',
        'Language should change to German'
      )
      ...
      assert.strictEqual(
        i18next.language,
        'en',
        'Language should change back to English'
      )
    ...
    await it('defaults to fallback language for invalid language code', async () => {
      ...
      assert.ok(
        i18next.language === 'en' || i18next.language === originalLng,
        'Should use fallback language or keep original'
      )
