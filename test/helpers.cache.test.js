...
import { getCachedBurialSiteTypeById, getCachedBurialSiteTypes, getCachedBurialSiteTypesByBurialSiteType } from '../helpers/cache/burialSiteTypes.cache.js';
import { getCachedCommittalTypeById, getCachedCommittalTypes } from '../helpers/cache/committalTypes.cache.js';
import { getCachedContractTypeByContractType, getCachedContractTypeById, getCachedContractTypes } from '../helpers/cache/contractTypes.cache.js';
...
await describe('helpers.cache', async () => {
    ...
    await describe('Burial Site Statuses', async () => {
        await it('returns Burial Site Statuses', () => {
            ...
            for (const burialSiteStatus of burialSiteStatuses) {
                ...
                assert.strictEqual(burialSiteStatus.burialSiteStatusId, byId?.burialSiteStatusId, 'Expected burial site status ID to match');
                const byName = getCachedBurialSiteStatusByBurialSiteStatus(burialSiteStatus.burialSiteStatus);
                assert.strictEqual(burialSiteStatus.burialSiteStatus, byName?.burialSiteStatus, 'Expected burial site status to match');
            }
        });
    ...
    await describe('Burial Site Types', async () => {
        await it('returns Burial Site Types', () => {
            ...
            assert.notStrictEqual(burialSiteTypes.length, 0, 'Expected burial site types to be cached');
    ...
    await describe('Committal Types', async () => {
        await it('returns Committal Types', () => {
            ...
            assert.notStrictEqual(committalTypes.length, 0, 'Expected committal types to be cached');
            for (const committalType of committalTypes) {
                const byId = getCachedCommittalTypeById(committalType.committalTypeId);
                assert.strictEqual(committalType.committalTypeId, byId?.committalTypeId, 'Expected committal type ID to match');
            }
        });
    ...
    await describe('Contract Types', async () => {
        await it('returns Contract Types', () => {
            ...
            assert.notStrictEqual(contractTypes.length, 0, 'Expected contract types to be cached');
            for (const contractType of contractTypes) {
                const byId = getCachedContractTypeById(contractType.contractTypeId);
                assert.strictEqual(contractType.contractTypeId, byId?.contractTypeId, 'Expected contract type ID to match');
                const byName = getCachedContractTypeByContractType(contractType.contractType);
                assert.strictEqual(contractType.contractType, byName?.contractType, 'Expected contract type to match');
            }
        });
    ...
    await describe('Interment Container Types', async () => {
        await it('returns Interment Container Types', () => {
            ...
            assert.notStrictEqual(intermentContainerTypes.length, 0, 'Expected interment container types to be cached');
            for (const intermentContainerType of intermentContainerTypes) {
                const byId = getCachedIntermentContainerTypeById(intermentContainerType.intermentContainerTypeId);
                assert.strictEqual(intermentContainerType.intermentContainerTypeId, byId?.intermentContainerTypeId, 'Expected interment container type ID to match');
            }
        });
        await it('returns undefined with a bad intermentContainerTypeId', () => {
            const byBadId = getCachedIntermentContainerTypeById(badId);
            assert.strictEqual(byBadId, undefined, 'Expected undefined for bad intermentContainerTypeId');
        });
    });
    await describe('Interment Depths', async () => {
        await it('returns Interment Depths', () => {
            ...
            assert.notStrictEqual(intermentDepths.length, 0, 'Expected interment depths to be cached');
            for (const intermentDepth of intermentDepths) {
                const byId = getCachedIntermentDepthById(intermentDepth.intermentDepthId);
                assert.strictEqual(intermentDepth.intermentDepthId, byId?.intermentDepthId, 'Expected interment depth ID to match');
            }
        });
    ...
    await describe('Service Types', async () => {
        await it('returns Service Types', () => {
            ...
            assert.notStrictEqual(serviceTypes.length, 0, 'Expected service types to be cached');
            for (const serviceType of serviceTypes) {
                const byId = getCachedServiceTypeById(serviceType.serviceTypeId);
                assert.strictEqual(serviceType.serviceTypeId, byId?.serviceTypeId, 'Expected service type ID to match');
                const byName = getCachedServiceTypeByServiceType(serviceType.serviceType);
                assert.strictEqual(serviceType.serviceType, byName?.serviceType, 'Expected service type to match');
            }
        });
    ...
    await describe('Settings', async () => {
        ...
        await it('returns a Setting by key', () => {
            ...
            assert.strictEqual(setting.settingKey, 'aliases.externalReceiptNumber', 'Expected setting key to match');
        });
        await it('returns a setting value by key', () => {
            const settingValue = getCachedSettingValue('aliases.externalReceiptNumber');
            assert.strictEqual(typeof settingValue, 'string', 'Expected setting value to be a string');
        });
    });
    await describe('Work Order Types', async () => {
        await it('returns Work Order Types', () => {
            ...
            assert.notStrictEqual(workOrderTypes.length, 0, 'Expected work order types to be cached');
            for (const workOrderType of workOrderTypes) {
                const byId = getCachedWorkOrderTypeById(workOrderType.workOrderTypeId);
                assert.strictEqual(workOrderType.workOrderTypeId, byId?.workOrderTypeId, 'Expected work order type ID to match');
            }
        });
    ...
    await describe('Work Order Milestone Types', async () => {
        await it('returns Work Order Milestone Types', () => {
            ...
            assert.notStrictEqual(workOrderMilestoneTypes.length, 0, 'Expected work order milestone types to be cached');
            for (const workOrderMilestoneType of workOrderMilestoneTypes) {
                const byId = getCachedWorkOrderMilestoneTypeById(workOrderMilestoneType.workOrderMilestoneTypeId);
                assert.strictEqual(workOrderMilestoneType.workOrderMilestoneTypeId, byId?.workOrderMilestoneTypeId, 'Expected work order milestone type ID to match');
                const byName = getCachedWorkOrderMilestoneTypeByWorkOrderMilestoneType(workOrderMilestoneType.workOrderMilestoneType);
                assert.strictEqual(workOrderMilestoneType.workOrderMilestoneType, byName?.workOrderMilestoneType, 'Expected work order milestone type to match');
            }
        });
        await it('returns undefined with a bad workOrderMilestoneTypeId', () => {
            const byBadId = getCachedWorkOrderMilestoneTypeById(badId);
            assert.strictEqual(byBadId, undefined, 'Expected undefined for bad workOrderMilestoneTypeId');
        });
        await it('returns undefined with a bad workOrderMilestoneType', () => {
            const byBadName = getCachedWorkOrderMilestoneTypeByWorkOrderMilestoneType(badName);
            assert.strictEqual(byBadName, undefined, 'Expected undefined for bad workOrderMilestoneType');
        });
    });
});
