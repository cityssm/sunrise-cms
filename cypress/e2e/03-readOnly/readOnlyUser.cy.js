...
describe('Read Only User', () => {
    ...
    it('Redirects to Dashboard when attempting to create or update a cemetery', () => {
        ...
        cy.wait(200);
        cy.location('pathname', { timeout: pageLoadDelayMillis }).should('not.contain', 'new');
        cy.visit('/cemeteries/1/edit');
        cy.wait(200);
        cy.location('pathname', { timeout: pageLoadDelayMillis }).should('not.contain', 'edit');
    });
    // Burial Sites
    ...
    it('Redirects to Dashboard when attempting to create or update a burial site', () => {
        ...
        cy.wait(200);
        cy.location('pathname', { timeout: pageLoadDelayMillis }).should('not.contain', 'creator');
        cy.visit('/burialSites/gpsCapture');
        cy.wait(200);
        cy.location('pathname', { timeout: pageLoadDelayMillis }).should('not.contain', 'gpsCapture');
    });
    // Contracts
    ...
    it('Redirects to Dashboard when attempting to access admin sections', () => {
        ...
        for (const adminPath of adminPaths) {
            cy.visit(adminPath);
            cy.wait(200);
            cy.location('pathname', { timeout: pageLoadDelayMillis }).should('not.contain', '/admin');
        }
    });
